                                ...
                                t_ivec** clb_opins_used_locally)
{
    ...
    float T_crit = 1.0; /* timing_critical */
...
boolean timing_driven_route_net(int inet, float pres_fac,
                                float max_criticality, float criticality_exp,
                                float astar_fac, float bend_cost,
                                float* net_slack, float* pin_criticality,
                                int* sink_order, t_rt_node** rt_node_of_sink,
                                float T_crit, float* net_delay)
{
    ...
    float largest_criticality = pin_criticality[sink_order[1]];
    ...
    for (itarget = 1; itarget <= num_sinks; ++itarget) {
        ...
        while (inode != target_node) {
            float old_tcost = rr_node_route_inf[inode].path_cost;
            float new_tcost = current->cost;

            float old_back_cost = 0.0;
...
static void update_rr_base_costs(int inet, float largest_criticality)
{
    ...
    float fanout = net[inet].num_pins - 1.0;
