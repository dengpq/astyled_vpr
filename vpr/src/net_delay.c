 ...
 * float net_delay[0...num_nets-1][1...num_pins-1] was used for NET_TIMING_DRIVEN_PLACE *
 ****************************************************************************************/
float** alloc_net_delay(struct s_linked_vptr** chunk_list_head_ptr)
{
    ...
    *chunk_list_head_ptr = NULL;
    ...
    float** net_delay = (float**)my_malloc(num_nets * sizeof(float*));

    int inet;
    for (inet = 0; inet < num_nets; ++inet) {
        float* tmp_ptr = (float*)my_chunk_malloc((net[inet].num_pins - 1) *
