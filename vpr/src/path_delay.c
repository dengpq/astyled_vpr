                                    ...
                                    t_subblock_data subblock_data)
{
     ...
     * the first two pin locations are used(input to pad is first, output of pad *
    ...
    /* index number of all block pins and its subblocks pins. When using C++ *
     * write the program, I can use map<int, int>. */
    num_tnodes = alloc_and_load_pin_mappings(&block_pin_to_tnode,
                                             &sblk_pin_to_tnode,
                                             subblock_data, num_uses_of_sblk_opin);

    /*==== Then initial tnode and tedge to create Timing_analyze_graph */
    alloc_and_load_tnodes_and_net_mapping(num_uses_of_clb_ipin, num_uses_of_sblk_opin,
                                          block_pin_to_tnode, sblk_pin_to_tnode,
    ...
    float** net_slack = alloc_net_slack();
    return (net_slack);
} /* end of float** alloc_and_load_timing_graph() */


static float** alloc_net_slack(void)
{
    /* Allocates the net_slack structure. Chunk allocated to save space. */
    /* float net_slack[0..num_nets-1][1..num_pins-1], and its value depended on *
     * float net_delay[0...num_nets-1][1...num_pins-1].                         */
    float** net_slack = (float**)my_malloc(num_nets * sizeof(float*));

    float* tmp_ptr;
    int inet;
    for (inet = 0; inet < num_nets; ++inet) {
        float* tmp_ptr = (float*)my_chunk_malloc((net[inet].num_pins-1) * sizeof(float),
                                                 &tedge_ch_list_head,
                                       ...
                                       int** num_uses_of_sblk_opin)
{
    ...
    int** block_pin_to_tnode = (int**)alloc_matrix(0, num_blocks-1, 0,
                                                   pins_per_clb-1, sizeof(int));

    int*** sblk_pin_to_tnode = (int***)my_malloc(num_blocks * sizeof(int**));

    int out_pin = subblock_lut_size; /* Input[0..subblock_lut_size-1], Output, CLK */
    int clk_pin = subblock_lut_size + 1;
    ...
    for (iblk = 0; iblk < num_blocks; ++iblk) {
        if (block[iblk].type == CLB) {
            /* First do current netlist's block mapping */
            ...
            int num_subblocks = num_subblocks_per_block[iblk];
            sblk_pin_to_tnode[iblk] = (int**)alloc_matrix(0, num_subblocks-1,
                                                          0, subblock_lut_size+1,
                                                          sizeof(int));
            int isub = -1;
                                         ...
                                         t_subblock_data subblock_data)
{
    ...
    for (iblk = 0; iblk < num_blocks; ++iblk) {
        if (block[iblk].type != CLB) {
            ...
            num_uses_of_clb_ipin[iblk] = (int*)my_calloc(pins_per_clb, sizeof(int));
...
static void free_fanout_counts(int** num_uses_of_clb_ipin, int** num_uses_of_sblk_opin)
{
...
/* FIXME: allocate and load timing_graph node(for clb, inpad, outpad)and edges for Timing Analyze */
static void alloc_and_load_tnodes_and_net_mapping(int** num_uses_of_clb_ipin,
                                                  int** num_uses_of_sblk_opin,
                                                  int** block_pin_to_tnode,
                                                  int*** sblk_pin_to_tnode,
                                                  ...
                                                  t_timing_inf timing_inf)
{
    ...
    /* Attention, tedge in Timing-Analyze-Graph was depend no connection, *
     * not net. */
...
static void build_clb_tnodes(int iblk, int* n_uses_of_clb_ipin,
                             ...
                             int subblock_lut_size, int num_subs,
                             t_subblock* sub_inf,
                                     ...
                                     int** block_pin_to_tnode)
{
    ...
    tnode[inode].num_edges = num_edges;
    tnode[inode].out_edges = (t_tedge*)my_chunk_malloc(num_edges * sizeof(t_tedge),
                                                       &tedge_ch_list_head,
                                                       &tedge_ch_bytes_avail,
                                                       &tedge_ch_next_avail);
                                  ...
                                  int* next_sblk_opin_edge, int iblk)
{
     ...
     * in.                                                                      */
    int isub, ipin, inode, to_node, from_pin, to_pin;
    int iedge, num_edges;
    float ipin_to_sink_Tdel;
    t_tedge* tedge;
    boolean has_inputs;
    int out_pin = subblock_lut_size;
    int clk_pin = subblock_lut_size + 1;
...
/* FIXME: calculate T_arr and T_req for all tnodes in Timing_Analyze_Graph, *
...
float load_net_slack(float** net_slack, float target_cycle_time)
{
     ...
     * If a source-sink pair in timing-graph slack was 0, so it was edges in  *
     * critical_path.                                                        */
    ...
    int inode = -1;
    for (inode = 0; inode < num_tnodes; ++inode) {
        tnode[inode].T_arr = T_CONSTANT_GENERATOR; /* arrival_timing = -1000 */
    }

    /* Compute all arrival times with a breadth-first analysis from inputs to *
     * outputs. Also compute critical path (T_crit).                         */
    float T_crit = 0.0; /* critical_timing */

    /* Attention: Primary inputs arrive at T = 0.0 */
    int i = -1;
    int num_at_level = tnodes_at_level[0].nelem;
    for (i = 0; i < num_at_level; ++i) {
        inode = tnodes_at_level[0].list[i];
        tnode[inode].T_arr = 0.0;
    }

    /* T_arr: actual_arrival_time, Tdel: edge_delay_time, T_cycle: cycle_time,*
     * T_req: required_arrival_time                                          */
    float T_arr, Tdel, T_cycle, T_req;
    int ilevel, num_edges, iedge, to_node;
    t_tedge* tedge = NULL;
    /* use breath-first-search to calculate the T_arr, T_c*/
    for (ilevel = 0; ilevel < num_tnode_levels; ++ilevel) {
    ...
    if (target_cycle_time > 0.0) { /* User specified target cycle time */
        T_cycle = target_cycle_time;
    } else {                       /* Otherwise, target = critical path */
        T_cycle = T_crit;
    }

    /* Compute the all tnodes' T_req(required_arrival_times) with a backward  *
